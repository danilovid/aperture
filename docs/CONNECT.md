...
Claude Code speaks the Anthropic Messages API, which the gateway serves natively. Three environment variables, no code change.
...
export ANTHROPIC_CUSTOM_HEADERS='X-Mutegate-Agent: claude-code'
claude
```
...
```json
{
  "env": {
    ...
    "ANTHROPIC_CUSTOM_HEADERS": "X-Mutegate-Agent: claude-code"
  }
}
```
...
[curl documentation](https://github.com/danilovid/mutegate/blob/main/docs/API.md)
