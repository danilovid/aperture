...
- [ ] Вкладки: Overview (текущий дашборд + DLP KPI), DLP Events (лента
      инцидентов с фильтрами и деталями), Policies (тумблеры групп, действия,
      кастомные правила, live-превью dry-run), Settings/Keys (текущее).
- [ ] Состояния: skeleton / empty / error; тёмная+светлая тема.
